...
export default function Home() {
  ...
  const getCurrentTime = () => {
    ...
    const localTime = new Date(now.getTime() - timeZoneOffset * 60 * 1000); // Adjust for offset
    ...
    return localTime.toLocaleTimeString([], {
      ...
      timeZoneName: "short", // Ensures timezone abbreviation
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    const prompt = `Hey! You're talking to my friend ${name}, who's been bedrotting and has ${timeframe} to get back on track! Here's what ${name} has to get done':\n` +
      ...
      `\n\nIn a fun, gen-z vibe, give them a structured, realistic schedule based on the current time (${name}'s timezone is ${getCurrentTime}) and amount of time they have to work in this schedule format:\n[Greeting message]\n[Current time (rounded to the nearest 5th minute)]\n[...]. (Note: use HTML formatting; when you want to bold, DON'T USE **)`;
