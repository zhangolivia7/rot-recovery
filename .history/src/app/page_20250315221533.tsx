    ...
    <div className="h-screen overflow-y-scroll snap-y snap-mandatory">
      ...
      <section className="h-screen flex flex-col items-center justify-center snap-start">
        <div id="form">
              ...
              {tasks.map(task => (
                <li key={task.id} className="flex items-center space-x-2">
                  ...
                  <input
                    ...
                    className={`border-b border-gray-400 focus:outline-none bg-transparent ${task.completed ? "line-through text-gray-400" : ""}`}
                  />

                  {/* Delete Button */}
                  <button onClick={() => deleteTask(task.id)} className="text-red-500 text-lg">
                    ✕
                  </button>
                </li>
              ))}
          ...
          {/* Add Task Input (Press Enter to Add) */}
          <input
            className="border p-2 w-80 mt-2"
